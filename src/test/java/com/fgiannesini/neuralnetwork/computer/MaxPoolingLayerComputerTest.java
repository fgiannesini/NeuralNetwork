...
class MaxPoolingLayerComputerTest {
    ...
    void convolution_layer_no_padding_no_stride_one_channel_one_input() {
        ...
        MaxPoolingData input = new MaxPoolingData(Collections.singletonList(DoubleMatrix.ones(5, 5)), Collections.emptyList());
        LayerComputerVisitor layerComputerVisitor = new LayerComputerVisitor(input);
        layer.accept(layerComputerVisitor);
        DoubleMatrixAssertions.assertMatrices(((MaxPoolingData) layerComputerVisitor.getIntermediateOutputResult().getResult()).getDatas(), Collections.singletonList(DoubleMatrix.ones(3, 3)));
    }

    @Test
    void convolution_layer_padding_no_stride_one_channel_one_input() {
        MaxPoolingLayer layer = new MaxPoolingLayer(ActivationFunctionType.NONE, 3, 1, 1, 1, 10, 10, 8, 8);
        MaxPoolingData input = new MaxPoolingData(Collections.singletonList(DoubleMatrix.ones(5, 5)), Collections.emptyList());
        LayerComputerVisitor layerComputerVisitor = new LayerComputerVisitor(input);
        layer.accept(layerComputerVisitor);
        DoubleMatrixAssertions.assertMatrices(((MaxPoolingData) layerComputerVisitor.getIntermediateOutputResult().getResult()).getDatas(), Collections.singletonList(DoubleMatrix.ones(5, 5)));
    }
    ...
    void convolution_layer_no_padding_stride_one_channel_one_input() {
        ...
        MaxPoolingData input = new MaxPoolingData(Collections.singletonList(DoubleMatrix.ones(15, 15)), Collections.emptyList());
        LayerComputerVisitor layerComputerVisitor = new LayerComputerVisitor(input);
        layer.accept(layerComputerVisitor);
