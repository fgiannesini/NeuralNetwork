...
public class GradientDescent implements LearningAlgorithm {
  private NeuralNetworkModel neuralNetworkModel;
    ...
    private List<GradientDescentCorrection> launchBackwardComputation(List<GradientDescentLayerResult> gradientDescentLayerData, FloatMatrix y, FloatMatrix inputMatrix) {
        ...
        FloatMatrix dz = currentGradientDescentLayerResult.getaLayerResults().subi(y);
        FloatMatrix weightCorrection = dz.mmul(nextGradientDescentLayerResult.getaLayerResults().transpose()).divi(inputCount);
        FloatMatrix biasCorrection = dz.columnSums().divi(inputCount);
        ...
        for (int layerIndex = 1; layerIndex < reverseGradientDescentLayerData.size() - 1; layerIndex++) {
            ...
            dz = weightCorrection.transpose().mmul(dz).muli(activationFunction.apply(currentGradientDescentLayerResult.getzLayerResults()));
            weightCorrection = dz.mmul(nextGradientDescentLayerResult.getaLayerResults().transpose()).divi(inputCount);
            biasCorrection = dz.columnSums().divi(inputCount);
            gradientDescentCorrections.add(new GradientDescentCorrection(weightCorrection, biasCorrection));
        }
        ...
        ActivationFunctionApplier activationFunction = currentGradientDescentLayerResult.getActivationFunctionType().getActivationFunction();
        dz = weightCorrection.transpose().mmul(dz).muli(activationFunction.apply(currentGradientDescentLayerResult.getzLayerResults()));
        weightCorrection = dz.mmul(inputMatrix.transpose()).divi(inputCount);
        biasCorrection = dz.columnSums().divi(inputCount);
        gradientDescentCorrections.add(new GradientDescentCorrection(weightCorrection, biasCorrection));
